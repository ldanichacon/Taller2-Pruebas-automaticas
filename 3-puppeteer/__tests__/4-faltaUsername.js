...
describe ('Registro', ()=>{
	...
	beforeAll(async ()=>{
		...
		await page.screenshot({path: '4-faltaUsername.png'});
		await page.click('.btn-primary');
	},timeout);
	it('Falta Username', async () => {
		await expect(page).toMatch('Username is required',{ timeout: timeout });
		await page.screenshot({path: '4-faltaUsernameResultado.png'});
  });
});